...
--*/
{
    ...
    if (NT_SUCCESS(ntStatus))
    {
        DPF(D_VERBOSE, ("[CMiniportWaveRTStream::RegisterNotificationEvent] Successfully registered notification event 0x%llx with PWM", (SIZE_T)NotificationEvent));
    }
    else
    {
    ...
    if (NT_SUCCESS(ntStatus))
    {
        ...
        DPF(D_VERBOSE, ("[CMiniportWaveRTStream::RegisterNotificationEvent] Notification event registered: 0x%llx", (SIZE_T)NotificationEvent));
    }
...
--*/
{
    ...
    if (NT_SUCCESS(ntStatus))
    {
        DPF(D_VERBOSE, ("[CMiniportWaveRTStream::UnregisterNotificationEvent] Successfully unregistered notification event 0x%LLX", (SIZE_T)NotificationEvent));
    }
    else
    {
    ...
    if (!IsListEmpty(&m_NotificationList))
    {
        ...
        while (currentNotificationListEntry != &m_NotificationList)
        {
            ...
            if (currentNotfication->NotificationEvent == NotificationEvent)
            {
                ...
                DPF(D_VERBOSE, ("[CMiniportWaveRTStream::UnregisterNotificationEvent] Notification event (0x%llx) unregistered", (SIZE_T)NotificationEvent));
