    ...
    if (NT_SUCCESS(status))
    {
        ...
        if (NT_SUCCESS(status))
        {
            ...
            if (NT_SUCCESS(status))
            {
                ...
                TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_INIT, "Build CBs for %d audio packets. CBs @ 0x%llx (phys: 0x%08x) Packet size: %d, First chunk size: %d",
                    deviceContext->dmaNumPackets, (SIZE_T)deviceContext->dmaCb, deviceContext->dmaCbPa.LowPart, packetSize, packetFirstChunkSize
                    );
                ...
                for (currentCb = deviceContext->dmaCb, packetIndex = 0; packetIndex < deviceContext->dmaNumPackets; packetIndex++, currentCb++)
                {
                    ...
                    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_INIT, "First CB packet %d @ 0x%llx (phys: 0x%08x) - NEXTCONBK: 0x%08x, TI: 0x%08x, SOURCE_AD: 0x%08x, DEST_AD: 0x%08x, TXFR_LEN: 0x%08x (%d)",
                        packetIndex, (SIZE_T)currentCb, pa.LowPart, currentCb->NEXTCONBK, currentCb->TI, currentCb->SOURCE_AD, currentCb->DEST_AD, currentCb->TXFR_LEN, currentCb->TXFR_LEN
                        );
                    currentCb++;
                    ...
                    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_INIT, "Second CB packet %d @ 0x%llx (phys: 0x%08x) - NEXTCONBK: 0x%08x, TI: 0x%08x, SOURCE_AD: 0x%08x, DEST_AD: 0x%08x, TXFR_LEN: 0x%08x (%d)",
                        packetIndex, (SIZE_T)currentCb, pa.LowPart, currentCb->NEXTCONBK, currentCb->TI, currentCb->SOURCE_AD, currentCb->DEST_AD, currentCb->TXFR_LEN, currentCb->TXFR_LEN
                        );
                    ...
                    deviceContext->dmaPacketLinkInfo[packetIndex].LinkValue = (SIZE_T)((PCHAR)(deviceContext->dmaCbPa.LowPart) + packetIndex * 2 * sizeof(DMA_CB));;
                    if (packetIndex == 0)
                    {
...
--*/
{
    ...
    if (NT_SUCCESS(status))
    {
        ...
        TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_IO, "Notification event registered: 0x%llx, Current process: 0x%llx", (SIZE_T)*notificationEvent, (SIZE_T)IoGetCurrentProcess());
        ...
        if (!IsListEmpty(&deviceContext->notificationList))
        {
            ...
            while (currentNotificationListEntry != &deviceContext->notificationList)
            {
                ...
                if (currentNotfication->NotificationEvent == *notificationEvent)
                {
                    ...
                    TraceEvents(TRACE_LEVEL_INFORMATION, TRACE_IO, "Notification event unregistered: 0x%llx", (SIZE_T)*notificationEvent);
